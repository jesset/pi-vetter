...
- **install 场景静态降级是设计取舍而非可配规则**（#12）：降级发生在 evidence status 层（fail→info），早于规则映射，因此不受 `rules` 开关控制——这是刻意的：无基线时这两类命中几乎总是包的正当属性，与"信号可开关"的规则哲学（针对有据风险）作用层不同。若未来需要恢复硬信号，应加配置而非挪层。
