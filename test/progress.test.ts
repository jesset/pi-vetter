...
describe("ProgressTracker", () => {
  it("renders an idle line before any target is resolved", () => {
    const tracker = new ProgressTracker("Vetting packages");
    expect(tracker.lines()).toEqual(["Vetting packages…"]);
  });

  it("counts completed packages and shows the current one", () => {
    const tracker = new ProgressTracker("Vetting packages");
    tracker.start(4);
    expect(tracker.lines()).toEqual(["Vetting packages (0/4)"]);

    tracker.item("pi-web-access");
    expect(tracker.lines()).toEqual(["Vetting packages (0/4)", "→ pi-web-access"]);

    tracker.tick();
    tracker.item("pi-mcp-adapter");
    expect(tracker.lines()).toEqual(["Vetting packages (1/4)", "→ pi-mcp-adapter"]);
  });
  ...
  it("shows completion when all packages finished", () => {
    ...
    tracker.item("b");
    tracker.tick();
    expect(tracker.lines()).toEqual(["Vetting packages (2/2) done"]);
  });
});
