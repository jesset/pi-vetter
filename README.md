...
- **ALLOW** — no risk signal found from enabled scanners (the report carries the qualifier: this does not prove the package is safe)
...
| L1 | `provenance` | npm attestations: full sigstore signature-chain verification against a vendored public TrustedRoot, plus declared-repo conflict detection; verified bundles yield `provenance:verified`. Missing attestations stay informational unless `provenance.required` is set |
| L2 | `static` | pattern scan of code files: credential access, obfuscation, prompt-injection markers, eval family (ASK in both scenarios) plus dynamic module resolution (concatenated / base64-decoded / variable require-import); pre-existing hits are info, new hits are findings; in the install scenario (no baseline) credential/obfuscation hits stay informational while prompt-injection remains a hard signal |
...
| L3 | `socket` | Socket.dev package alerts (gptMalware, installScripts, obfuscatedFile, typosquatting...); high-risk alerts → ASK (`socket-flagged`). Disabled by default — note the free-tier token carries a per-call quota balance (a purl scan consumes a chunk of it), so expect routine quota-exhaustion (which caps verdicts at ASK) once the balance runs dry |
...
Rules map evidence to verdicts and can be toggled individually in the config file (`ask.new-lifecycle-script: false` etc.). Current DENY rules: `malicious-package`, `provenance-conflict`, `vt-detections`. Current ASK rules: `known-vulnerability`, `new-lifecycle-script`, `maintainer-change`, `new-dependency-flagged`, `new-network-endpoint`, `new-child-process`, `credential-access`, `dynamic-code-execution`, `transitive-risk`, `obfuscation`, `prompt-injection-marker`, `young-package`, `rapid-release`, `deprecated-candidate`, `provenance-missing` (only when `provenance.required` is on). Rules you disable are disclosed in the report Notes so an ALLOW never masquerades as a full scan.
...
- Deep dependency scanning (`dependencies.*` config, on by default, depth 2 / 20 packages) downloads and statically scans transitive dependency tarballs — each dependency resolves to the highest published version inside its declared range (falling back to `latest` when the range has no plain-semver shape or no in-range version exists, so the scanned tarball can occasionally differ from what npm would install); hits are attributed per dependency: risky labels (credentials, obfuscation, prompt injection, dynamic code) escalate to an ASK-level transitive-risk rule, ordinary Node.js API usage stays informational.
