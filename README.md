...
| L2 | `static` | pattern scan of code files: credential access, obfuscation, prompt-injection markers, eval family and dynamic module resolution (concatenated / base64-decoded / variable require-import); pre-existing hits are info, new hits are findings; in the install scenario (no baseline) credential/obfuscation hits stay informational while prompt-injection and dynamic code execution remain hard signals |
